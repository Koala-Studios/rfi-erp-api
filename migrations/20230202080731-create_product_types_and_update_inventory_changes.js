...
module.exports = {
  async up(db, client) {
    ...
    const product_type = await db.collection("product_types").findOne({'code' : {$eq : (inventoryItem.product_code).slice(0,2) } })
      if( product_type) {
        await db
        .collection("inventory").updateOne({_id: inventoryItem._id}, {$set: {'rating':null, 'avoid_recur': product_type.avoid_recur, 'is_raw': product_type.is_raw, 'product_type': {'name': product_type ? product_type.name : 'Not Found', 'code': product_type ? product_type.code : 'Not Found', '_id': product_type ? product_type._id : 'Not Found'}}});
