...
export const listPurchases = async (
    ...
    const list = await PurchaseOrder.paginate(
      {},
      { page: listParams.page, limit: listParams.count, leanWithId: true,sort: {"date_purchased": -1}}
    );
...
export const proccessPurchaseRow = async (row:IOrderItemProcess, quarantine:boolean):Promise<ILogicResponse> => {
    let _status:number;
    let _message:string = "";
    const purchase_order = await PurchaseOrder.findOne({ "order_items._id": row._id });
    const item_index = purchase_order.order_items.findIndex((item) => item._id.toString() === row._id.toString())
    const inv_item = await Inventory.findById(row.product_id);
    
    console.log("po: ", purchase_order, "invitem: ", inv_item, "order_item: " + purchase_order.order_items[item_index] )
    ...
    } else { //TODO: Change this step to ONLY send to QC. QC will then call the receiveItem function on its own
...
export const receiveItem = async (row:IOrderItemProcess):Promise<ILogicResponse> => {
    
    let _status:number;
    let _message:string = "";
    const inv_item = await Inventory.findOneAndUpdate({_id: row.product_id}, { $inc: {
    ...
    _status = status.OK
    return {status: _status, data:{message: "Successfully Added To Inventory",res: null}};
}


export const confirmPurchase = async (purchase:IPurchaseOrder):Promise<ILogicResponse> => {
    const po_id = purchase._id;
    
    purchase.status = (purchase.shipping_code != null && purchase.shipping_code != "") ? orderStatus.AWAITING_ARRIVAL : orderStatus.AWAITING_SHIPPING
    const _purchase = await PurchaseOrder.findOneAndUpdate({ _id: po_id }, purchase);
 
    for (const item of _purchase.order_items) {
        await Inventory.findOneAndUpdate({_id: item.product_id}, {$inc: {"stock.on_order": item.purchased_amount}})
    }
